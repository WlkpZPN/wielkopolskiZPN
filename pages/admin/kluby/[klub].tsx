  ...
  const markDebt = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setLoading('');
        if (newDebt) {
          toast.success(`Klub ${clubData.internal_id} został oznaczony jako zadłużony`);
        } else {
          toast.success(`Usunięto zadłużenie dla klubu ${clubData.internal_id}`);
        }
    ...
    <AdminLayout userData={authData} view="kluby">
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <span style={{ marginLeft: '16px' }}>{clubData.active ? 'aktywny' : 'nieaktywny'}</span>
        </div>
