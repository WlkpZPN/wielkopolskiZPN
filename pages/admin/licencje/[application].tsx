...
const Application = ({ clubData, authData, settings }) => {
      ...
      case 6:
        ...
        return (
          <>
            ...
            <ErrorMessage>
              {clubData.applications[0].invoice_required && !clubData.invoice_url
        ...
        return (
          <div style={{ display: 'flex' }}>
            <div>
              ...
              <LicenseButton authData={authData} isAdmin={true} clubData={clubData} />
            </div>
  ...
  const renderButtons = () => {
    switch (clubData.applications[0].statuses.id) {
      ...
      case 6:
        ...
        return (
          <div>
            <PrimaryButton color="warningDark" hoverColor="warning" onClick={resendLink}>
  ...
  const resendLink = async () => {
    const amount =
      clubData.applications[0].youth_groups_possession === 'nie posiadamy zespołów'
        ? renderMainAmount(clubData.leauge, settings, clubData.applications[0].number_of_seasons) +
  ...
  return (
    <AdminLayout view="wnioski" userData={authData}>
      <CorrectModal
        userID={authData.id}
        internalId={clubData.applications[0] ? clubData.applications[0].internal_id : ''}
      ...
      <RejectModal
        userID={authData.id}
        internalID={clubData.applications[0] ? clubData.applications[0].internal_id : ''}
        applicationID={clubData.applications[0] ? clubData.applications[0].id : ''}
      ...
      >
        <div style={{ display: 'flex' }}>
          ...
          >
            <ApplicationStatus size="32px" status={clubData.applications[0].statuses.name} />
            <span style={{ marginLeft: '16px' }}>{clubData.applications[0].statuses.name}</span>
          </div>
        </div>
        <div style={{ display: 'flex' }}>
          ...
          <PrimaryButton
            onClick={() => router.push(`/admin/licencje/historia/${clubData.applications[0].id}`)}
            style={{ margin: '0 6px' }}
          >
            Historia zmian
          </PrimaryButton>
