...
"
              ...
              <NumericInput
                ...
                onChange={(e) => handleChange(e.target.rawValue, 'numberOfYouthGroups', 3)}
                suffix=""
                placeholder="0"
              />
            </Label>
...
"
              ...
              <NumericInput
                ...
                onChange={(e) => handleChange(e.target.rawValue, 'shareOfYouthGroups', 3)}
                suffix=""
                placeholder="0"
              />
            </Label>
          ...
          <>
            <Label>
              ...
              <Input
                ...
                onChange={(e) => handleChange(e.target.value, 'clubAgreementName', 3)}
              />
            </Label>
            <Paragraph>
  ...
  return (
    <FormTemplate onChange={() => clearErrors('stepThree')} width="80%">
      {context.formData.stepOne.leauge == 'futsal' ? <DisabledStepInfo setStep={setStep} /> : null}
      <Fieldset disabled={!fileEdit}>
        <Label>
          Zespoły młodzieżowe
          <Select
            value={formData.youthGroupsPossession}
            onChange={(e) => handleChange(e.target.value, 'youthGroupsPossession', 3)}
          >
            ...
            <option value="porozumienie na szkolenie">
              Nie posiadamy własnych zespołów młodzieżowych. Posiadamy porozumienie na szkolenie
              młodzieży z innym klubem.
            </option>
            <option value="nie posiadamy zespołów">
              Nie posiadamy własnych zespołów młodzieżowych. Deklarujemy się uiścić opłatę dodatkową
              do Wielkopolskiego ZPN.
            </option>
            <option value="wystepujemy w rozgrywkach klasy A">
              Nie posiadamy własnych zespołów młodzieżowych. Występujemy w rozgrywkach Klasy A,
              Klasy B lub ligi młodzieżowej.
            </option>
          </Select>
        </Label>
...
"
          ...
          handleChange={() => handleChange(!formData.medicalDeclaration, 'medicalDeclaration', 3)}
      ...
      <div style={{ marginBottom: '32px' }}>
        <PrimaryButton
          ...
          onClick={() => (show_buttons ? handleStepChange('previous') : setStep(2))}
        >
          Cofnij
        </PrimaryButton>
        {show_buttons ? (
