...
export const generatePdf = async (clubData, date = null, dwn = true) => {
  ...
  const fontBytes2 = await fetch('https://pdf-lib.js.org/assets/ubuntu/Ubuntu-B.ttf').then((res) =>
    res.arrayBuffer(),
  );
  ...
  page.drawText('Decyzja Komisji ds. Licencji Klubowych \n Wielkopolskiego Związku Piłki Nożnej', {
    //x: 170,
    x: 180,
    y: 710,
    font: bold,
  ...
  textField.setText(`${clubData.name.replace(/\n/g, ' ')} ${clubData.address.replace(/\n/g, ' ')}`);
  ...
  if (clubData.leauge != 'futsal') {
    ...
    page.drawText('licencyjnych dla klubów IV ligi i klas niższych Wielkopolskiego ZPN na', {
      x: 50,
      y: 425,
      font: regular,
      size: 10,
    ...
    page.drawText('wniosku wraz z załącznikami i uzupełnieniami Komisja ds. Licencji Klubowych', {
      x: 50,
      y: 395,
      font: regular,
      size: 10,
  ...
  const textWidth = regular.widthOfTextAtSize('piłki nożnej w sezonach rozgrywkowych ', 10);
  ...
  if (clubData.leauge == 'futsal') {
    ...
    application.sport_facilities.forEach((object, index, arr) => {
      if (object.name) {
        objectsNames.push(`${object.name} ${object.address || ''}, ${object.city} \n`);
      }
    });
  }
  ...
  if (application.status_id === 10) {
    ...
    page.drawText(`na podstawie art. 3.3.2 Przepisów licencyjnych.`, {
      x: 50,
      y: 180,
      font: regular,
      size: 10,
